...
class BayesianLinearRegression:
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray, *args, **kwargs):
        ...
        if (self.state['trained'] and 
            ...
            self._prepare_optim()
            self.init_bounds_and_guess(*args, **kwargs)
    ...
    def predict(self, 
        ...
        Args:
            X (np.ndarray): Input data of shape (n_samples, n_features)
            return_std (bool): If True, return standard deviation of prediction
        ...
        if return_std:
            ...
            return (y_pred, y_std)      # type:ignore
    ...
    def _prepare_optim(self):
        ...
        opt_x = ctools.struct_symSX([
            ctools.entry("W", shape=(self.n_phi, self.n_y)),
            ctools.entry("Sigma_e_inv_bar", shape=sigma_e_sparsity),
            ctools.entry("log_alpha")
        ])
        opt_p = ctools.struct_symSX([
        ...
        Sigma_w_inv = cas.exp(-opt_x['log_alpha'])*Sigma_e_inv

        sum_dy2 = (
           + cas.trace(Sigma_e_inv@opt_x['W'].T@opt_p['XT_X']@opt_x['W'])
           -2 * cas.trace(opt_x['W']@Sigma_e_inv@opt_p['XT_y'].T)
           + cas.trace(Sigma_e_inv@opt_p['yT_y'])
        )
        J += (1/2)*sum_dy2
        ...
        sum_dw2 = cas.trace(Sigma_w_inv@opt_x['W'].T@opt_x['W'])
        ...
        self.Solver = cas.nlpsol('S', 'ipopt', nlp)
